...
Add an opt-in TOON (Token-Oriented Object Notation) encoding for tool results returned by the (by-then Python) MCP server, to reduce token usage for large/tabular outputs (e.g. window lists, tab lists, permission reports) consumed by LLM clients.
...
- Add TOON encoding as a new output helper (parallel to the Python equivalents of `textResult`/`untrustedResult`), gated by a tool argument or server capability — not a silent default-behavior change.
...
- Add/adjust pytest coverage and update the README tool table if `inputSchema` changes on any tool.
